  ...
  document.addEventListener('click', function(e){
    ...
    if(target.classList && target.classList.contains('habit-checkbox')){
      ...
      try{
        ...
        console.log('habit-tracker: before toggle', {id, isNowDone, rawMapKeys: Object.keys(map), arrBefore: arr.slice(0)});
        if(isNowDone){
        ...
        try{ console.log('habit-tracker: after toggle', {id, isNowDone, newMapKeys: Object.keys(map), arrAfter: arr.slice(0), saved: JSON.parse(localStorage.getItem(KEY))}); }catch(e){}
