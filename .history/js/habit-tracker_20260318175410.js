(function(){
  ...
  document.addEventListener('click', function(e){
    ...
    if(target.classList && target.classList.contains('habit-checkbox')){
      ...
      const today = (new Date()).toISOString().slice(0,10);
      // toggle visual
      const isNowDone = target.classList.toggle('done');
      try{
        const KEY = 'mindtrace_habit_completions_v1';
        const raw = localStorage.getItem(KEY);
        const map = raw ? JSON.parse(raw) : {};
        const arr = Array.isArray(map[id]) ? map[id] : [];
        console.debug('habit-tracker: before toggle', {id, isNowDone, rawMapKeys: Object.keys(map), arrBefore: arr.slice(0)});
        if(isNowDone){
        ...
        try{ console.debug('habit-tracker: after toggle', {id, isNowDone, newMapKeys: Object.keys(map), arrAfter: arr.slice(0), saved: JSON.parse(localStorage.getItem(KEY))}); }catch(e){}
