document.addEventListener('DOMContentLoaded', function () {
  ...
  (function attachRewardSystem() {
    ...
    // session selections (per-card). load persisted completed as initial completed set
    const persistedCompleted = new Set(loadCompleted());
    const sessionSelected = new Set(persistedCompleted); // start with persisted completed if any
    ...
    (function initCheckboxStates(){
      document.querySelectorAll('.habits-list .habit-card').forEach(card => {
        ...
        // if this habit was already persisted as completed today, show completed state
        if (persistedCompleted.has(name)) {
          card.classList.add('completed-today');
          btn.setAttribute('aria-pressed', 'true');
        } else {
          // otherwise ensure not marked completed; selection is per-card only
          card.classList.remove('completed-today');
          btn.setAttribute('aria-pressed', 'false');
          card.classList.remove('selected-today');
        }
      });
    })();
    ...
    document.addEventListener('click', function(e){
      ...
      // toggle only this card's session selection (visual 'selected')
      if (sessionSelected.has(name)) {
        sessionSelected.delete(name);
        card.classList.remove('selected-today');
      } else {
        sessionSelected.add(name);
        card.classList.add('selected-today');
      }

      // update aria-pressed to reflect selection for this button
      btn.setAttribute('aria-pressed', sessionSelected.has(name) ? 'true' : 'false');

      const allNames = todays.map(t=>t.name);
      const allSelected = allNames.every(n => sessionSelected.has(n));

      const rewardGivenKey = 'reward_given_' + todayKey;
      const award = 10;

      if (allSelected) {
        // when all selected, mark each as completed (visual), persist, and award points
        ...
        if (localStorage.getItem(rewardGivenKey)) {
          ...
          document.querySelectorAll('.habits-list .habit-card').forEach(card => {
            card.classList.remove('completed-today');
            // keep per-card selection state as-is (sessionSelected controls it)
          });
        }
      }
    });
