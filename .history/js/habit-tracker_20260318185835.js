(function(){
  ...
  document.addEventListener('click', function(e){
    ...
    if(target.classList && target.classList.contains('habit-checkbox')){
      ...
      const today = (new Date()).toISOString().slice(0,10);
      // Wait briefly for other scripts (reminders.js) to update UI, then persist only if the stored state differs
      setTimeout(function(){
        try{
          ...
          console.log('habit-tracker: post-delay state', {id, isNowDone, arrBefore: arr.slice(0)});
          ...
          if(isNowDone && !hasToday){
            ...
            try{ console.log('habit-tracker: saved (added)', {id, arrAfter: map[id]}); }catch(e){}
          } else if(!isNowDone && hasToday){
            map[id] = arr.filter(d=>d!==today);
            localStorage.setItem(KEY, JSON.stringify(map));
            completionsMap[id] = map[id];
            try{ console.log('habit-tracker: saved (removed)', {id, arrAfter: map[id]}); }catch(e){}
          } else {
            console.log('habit-tracker: no change needed', {id, isNowDone, hasToday});
          }
