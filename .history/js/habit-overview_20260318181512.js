(function(){
  document.addEventListener('DOMContentLoaded', function(){
    ...
    let comps = loadComps();
    // ignore any completions keys that don't match existing habit ids (cleanup view-time only)
    try{
      const valid = new Set(habits.map(h=>String(h.id)));
      Object.keys(comps).forEach(k=>{ if(!valid.has(k)) delete comps[k]; });
    }catch(e){/* ignore */}
    const container = document.getElementById('habit-progress-list');
    if(!container) return;

    // debug log to confirm script loaded and storage sizes
      // console.debug('habit-overview loaded — habits:', habits.length, 'completions keys:', Object.keys(comps).length);
    ...
    const infoText = `${last30} completed out of 30 days` + (last30===30 ? ' - 🎉 Perfect!' : '');
        // console.debug('habit', h.name, 'last30', last30, 'rate', rate);
    ...
    container.appendChild(el);
      ...
      // console.debug('progress-list children:', container.children.length, 'first child:', container.children[0] && container.children[0].outerHTML.slice(0,200));
  ...
  (function renderInsights(){
    ...
    `.trim());
      ...
      insightRoot.innerHTML = cards.join('');
        // console.debug('insight-cards children:', insightRoot.children.length, 'first card:', insightRoot.children[0] && insightRoot.children[0].outerHTML.slice(0,200));
  ...
  (function renderWeek(){
    ...
    days.forEach(day=>{
      ...
      // console.debug('week-grid children:', grid.children.length);
    }catch(err){ console.error('habit-overview: renderWeek error', err); }
  })();

    // listen for storage changes (from other pages) and reload to show updated completions
    try{
      window.addEventListener('storage', function(e){
        if(!e || !e.key) return;
        if(e.key === STORAGE_KEY || e.key === COMP_KEY){
          console.debug('habit-overview: storage event for', e.key, '— reloading to reflect changes');
