(function(){
  ...
  // sanitize accidental keys (e.g. "undefined") that may have been written by earlier bugs
  try{
    if(completionsMap && Object.prototype.hasOwnProperty.call(completionsMap, 'undefined')){
      delete completionsMap['undefined'];
  ...
  document.addEventListener('click', function(e){
    ...
    if(target.classList && target.classList.contains('habit-checkbox')){
      ...
      const today = (new Date()).toISOString().slice(0,10);
      // Other scripts (reminders.js) also toggle the class; wait a tick and read final state
      setTimeout(function(){
        try{
          const isNowDone = target.classList.contains('done');
          const KEY = 'mindtrace_habit_completions_v1';
          const raw = localStorage.getItem(KEY);
          const map = raw ? JSON.parse(raw) : {};
          const arr = Array.isArray(map[id]) ? map[id] : [];
          console.log('habit-tracker: before save', {id, isNowDone, rawMapKeys: Object.keys(map), arrBefore: arr.slice(0)});
          if(isNowDone){
          ...
          completionsMap[id]=arr;
          try{ console.log('habit-tracker: after save', {id, isNowDone, newMapKeys: Object.keys(map), arrAfter: arr.slice(0), saved: JSON.parse(localStorage.getItem(KEY))}); }catch(e){}
